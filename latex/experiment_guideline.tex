...
\section{Scope}
This is an inference-only evaluation of frozen Chronos-2 and Chronos-Bolt,
official TS-ICL and TiRex-2 inference packages, in-context TabPFN-TS, and four legacy TimeTensor
...
Chronos-2 and Chronos-Bolt, official TS-ICL 0.2.0 and TiRex-2 0.2.1 adapters, and in-context
TabPFN-TS. \\
...
TabPFN time features on/off where applicable. \\
\bottomrule
\end{tabularx}
\end{center}
...
All five frozen models use thin adapters over official packages. Chronos-2 and
Chronos-Bolt use version 2.0.1 of the official Chronos Forecasting package;
TS-ICL, TiRex-2, and
TabPFN-TS use \code{tsicl==0.2.0}, \code{tirex-2==0.2.1}, and
\code{tabpfn==6.3.1}. TS-ICL and TiRex-2 retain native preprocessing and
test-time behavior and select the median point forecast. The TSFM boundary
supplies raw targets for these two models, so the foundation workflow disables
the outer instance-normalization wrapper. Their sole shared aliases are
\code{chronos2}, \code{chronos_bolt}, \code{ts_icl}, \code{tirex2}, and
\code{tabpfn_ts}. Covariate configuration does not alter model identity, and
...
\begin{itemize}
  ...
  \item TiRex-2: \path{weights/tirex2/model-config.yaml} and
        \path{weights/tirex2/model.ckpt};
  \item TabPFN-TS:
        \path{weights/tabpfnts/tabpfn-v2.5-regressor-v2.5_default.ckpt}.
\end{itemize}
...
\item \code{03_covariates.slurm}: univariate and identity-covariate inference,
      with prepared known covariates available by override;
\item \code{04_foundation_models.slurm}: frozen Chronos-2, Chronos-Bolt,
      TS-ICL, TiRex-2, and TabPFN-TS with model-native preprocessing.
\end{enumerate}
\code{test} is Electricity at $504{:}168$, seed 1. \code{full} crosses
Electricity, Traffic, Solar, and Exchange Rate with $168{:}24$, $336{:}48$,
and $504{:}168$. \code{ultra} adds the documented extended datasets at the
shared $336{:}48$ and $504{:}168$ settings. Supersets select from the same
identity trees and reuse only signature-matched completed runs.

The foundation-model family uses Electricity at $504{:}168$ for \code{test};
its \code{full} profile crosses Electricity, Traffic, Solar, Weather, and
Exchange Rate with $336{:}48$ and $504{:}168$. Its report fixes Chronos-2 as
the default reference. \code{ultra} adds ETTh1 and $168{:}24$.
...
handling, covariates, and time-feature controls can be narrowed with the
...
The loader discovers \code{config.json} beside the CSV/tensor directory.
Portable settings are top-level, project overrides live under
\code{tsfm_evaluation}, explicit run values override both, and
\code{drop_users} is additive. The selected path and applied keys are logged.
Tensor directories follow the current TimeTensors contract: \path{values.pt},
optional \path{datetimes.pt} and \path{individual_ids.pt}, optional
\path{individual_context.pt} and \path{global_context.pt}, and
\path{dataset_metadata.json}. Static individual context is broadcast across
dates and global context across users.
...
counts, skips, and eligible query/window totals for the requested settings and
stride. The current revision inventory retains five hourly and seven daily
source configurations under the default limits; no 15-minute source qualifies.

The four independent family roots are \code{outputs/univariate},
\code{outputs/controls}, \code{outputs/covariates}, and
\code{outputs/foundation_models}. Their ordered identity is
\begin{verbatim}
outputs/family/dataset/L_H/backbone/covariate_mode/normalization/
  constant_policy/time_features/run_n/
\end{verbatim}
...
\path{outputs/reports/family/mode}, while \code{SELECTED_RUNS.txt} stores only
automatic or pinned exact repeats.
...
configuration table, equal-configuration marginal tables by dataset and by
setting, Chronos-2 strict paired-window win rates against the best aggregate
...
enabled, reports also export per-user mean-versus-dispersion scatters, window
error histograms, and horizon-error curves as PNG and PDF with a plot index.
...
TS-ICL, TiRex-2, or TabPFN-TS checkpoints or run full sweeps.
\end{document}
