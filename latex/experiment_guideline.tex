...
portable \code{missing_values} policy is applied after aggregation: omission
defaults to \code{zero}, which replaces NaNs with zero, while \code{error}
rejects them. Infinite values are always rejected. The selected path and
applied keys are logged. TSFM does not consume TimeTensors tensor caches.
...
baseline, and a report manifest. Covariate-family comparisons instead pair
each augmented configuration with the same backbone, task, and pipeline using
\code{covariate_mode=none}; they do not require a simple-model baseline. The
selected table metric may be any error
